...
def interpret_valuation(valuation_data: Dict, pe_ratio: float, sector: str, 
    ...
    if valuation_score.get('score'):
        ...
        if peg_overvalued and has_strong_fundamentals:
            # Case: PEG suggests overvaluation but strong fundamentals justify premium
            consolidated = f"PEG ratio of {peg_value:.2f} suggests overvaluation relative to growth, and the stock trades at a {premium:.1f}% premium to sector average. "
            ...
            consolidated += ". For quality companies, premium valuations can be sustained if fundamentals remain strong. "
            consolidated += f"The valuation score of {score:.1f}/10 reflects absolute P/E levels, but the premium reflects market recognition of quality and sustainable competitive advantages."
        ...
        if not (peg_overvalued and has_strong_fundamentals):
            ...
            if peg_value is not None and isinstance(peg_value, (int, float)):
                if peg_value < 1.0 and score < 5.0:
                    # PEG suggests fair/undervalued but P/E is expensive
                    interpretation_parts.append(f"Valuation reconciliation: PEG ratio of {peg_value:.2f} suggests fair valuation relative to earnings growth (PEG < 1.0 indicates growth-adjusted value). However, the overall valuation score of {score:.1f}/10 reflects that absolute P/E ratios are expensive. This means: (1) The company's growth rate justifies current multiples on a relative basis, but (2) absolute P/E levels are high and require continued exceptional growth to sustain. For investors, this suggests the stock may be fairly valued IF growth continues, but vulnerable if growth slows.")
        ...
        if growth_rate > 0:
            if 'target1' not in targets or targets['target1']['upside'] < growth_rate * 0.5:
                ...
                targets['target1'] = {
                    ...
                    'basis': f'Fundamental (50% of {growth_rate:.1f}% earnings growth) - Assumes price appreciation captures half of earnings growth rate over 12 months',
            ...
            if 'target2' not in targets or targets['target2']['upside'] < growth_rate * 0.75:
                ...
                targets['target2'] = {
                    ...
                    'basis': f'Fundamental (75% of {growth_rate:.1f}% earnings growth) - Assumes price appreciation captures three-quarters of earnings growth rate over 12 months',
            ...
            if 'target3' not in targets:
                ...
                targets['target3'] = {
                    'price': fundamental_target,
                    'upside': growth_rate,
                    'basis': f'Fundamental (100% of {growth_rate:.1f}% earnings growth) - Assumes price appreciation fully reflects earnings growth rate over 12 months',
...
"""
        ...
        # print(f"    📰 Headlines check: pos={len(positive_articles)}, neg={len(negative_articles)}, neu={len(neutral_articles)}")
        # print(f"    📰 news_sentiment keys: {list(news_sentiment.keys())}")
...
"""
    ...
    if quote:
        ...
        eps = quote.get('eps', 0)
        if pe_ratio and eps:
            # Simple forward-looking: If P/E is reasonable, suggest potential upside
